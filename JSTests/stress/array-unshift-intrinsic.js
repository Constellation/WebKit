...
// Length-0 Double with vectorLength == 0: fast path's checkEmptyCapacity branch
// must take the slow case. Newly constructed empty double arrays start with no
// vector storage.
(function () {
    function unshiftDouble(a, v) { return a.unshift(v); }
    noInline(unshiftDouble);
    for (var i = 0; i < testLoopCount; ++i) {
        // Seed the array as Double then truncate. JSC retains the Double indexing
        // type but the resulting butterfly's vectorLength can fall to 0 once the
        // length is reduced past the inline minimum, exercising the
        // vectorLength < 1 slow path on the next unshift.
        var array = [1.5];
        array.length = 0;
