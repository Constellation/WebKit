...
static JSPromiseContext::Field promiseContextInternalFieldIndex(BytecodeIntrinsicNode* node)
{
    ASSERT(node->entry().type() == BytecodeIntrinsicRegistry::Type::Emitter);
    if (node->entry().emitter() == &BytecodeIntrinsicNode::emit_intrinsic_promiseContextFieldPromise)
        return JSPromiseContext::Field::Promise;
    if (node->entry().emitter() == &BytecodeIntrinsicNode::emit_intrinsic_promiseContextFieldValues)
        return JSPromiseContext::Field::Values;
    if (node->entry().emitter() == &BytecodeIntrinsicNode::emit_intrinsic_promiseContextFieldRemainingElementsCount)
        return JSPromiseContext::Field::RemainingElementsCount;
    if (node->entry().emitter() == &BytecodeIntrinsicNode::emit_intrinsic_promiseContextFieldIndex)
        return JSPromiseContext::Field::Index;
    RELEASE_ASSERT_NOT_REACHED();
    return JSPromiseContext::Field::Promise;
}
...
RegisterID* BytecodeIntrinsicNode::emit_intrinsic_getPromiseContextInternalField(BytecodeGenerator& generator, RegisterID* dst)
{
    ArgumentListNode* node = m_args->m_listNode;
    RefPtr<RegisterID> base = generator.emitNode(node);
    node = node->m_next;
    RELEASE_ASSERT(node->m_expr->isBytecodeIntrinsicNode());
    unsigned index = static_cast<unsigned>(promiseContextInternalFieldIndex(static_cast<BytecodeIntrinsicNode*>(node->m_expr)));
    ASSERT(index < JSPromiseContext::numberOfInternalFields);
    ASSERT(!node->m_next);
...
RegisterID* BytecodeIntrinsicNode::emit_intrinsic_putPromiseContextInternalField(BytecodeGenerator& generator, RegisterID* dst)
{
    ArgumentListNode* node = m_args->m_listNode;
    RefPtr<RegisterID> base = generator.emitNode(node);
    node = node->m_next;
...
CREATE_INTRINSIC_FOR_BRAND_CHECK(isPromiseContext, IsPromiseContext)
